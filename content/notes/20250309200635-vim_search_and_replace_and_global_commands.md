...
The command divides in the next way:
...
The global command can also be quite useful, it works in a similar way to _substitute_.
The syntax is: `:[range]g/pattern/command`

Where **pattern** is what we are looking to match in the file and **command** is what we want to execute for each line matching the pattern.

As an example we can run `:g/error/d` to execute the command **d** _delete_ on every line that contains the word "error".
...
Note that although in this examples **d** is both a normal mode command and a command mode command, you can only execute command mode commands with the global command.
That said, you can use the normal prefix to execute a normal mode command like in this example: `:g/something/normal @a` where we execute the macro in register "a" for every line containing "something".
